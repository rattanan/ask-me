...
export function QuestionNote({ question, index }: QuestionNoteProps) {
  ...
  return (
    <motion.article
      ...
      className={`${colorClasses[question.color]} mb-5 inline-block w-[220px] break-inside-avoid rounded-2xl p-4 shadow-[0_18px_40px_rgba(15,23,42,0.13)] ${question.highlighted ? "ring-4 ring-blue-400" : ""}`}
      style={{ rotate: `${rotation}deg` }}
    >
      <div className="mb-3 flex items-center justify-between">
        <span className="text-3xl">{question.emoji}</span>
        {question.pinned ? <span className="rounded-full bg-white/70 px-2 py-1 text-xs font-bold text-zinc-700">Pinned</span> : null}
      </div>
