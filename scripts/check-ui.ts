...
async function main() {
 ...
 try {
  ...
  assert.equal(requests,1);await expect(page.getByRole("heading",{name:"Top 10 หมวดหมู่คำถาม",exact:true})).toBeVisible();await page.getByRole("heading",{name:"Top 10 หมวดหมู่คำถาม",exact:true}).scrollIntoViewIfNeeded();await page.screenshot({path:"/tmp/ask-me-insights-desktop.png",fullPage:true});
